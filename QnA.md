...
# How can I develop for Heliade OS?

 -- You can not be an "active developer" for Heliade OS as it's philosophy contains
 "worked on and made by a single person, Gretagen" however, you can contribute to it in multiple ways.

# How can I contribute to Heliade OS?
...
# Is Heliade OS fhs compliant?

-- Yes, Heliade OS is completely file system standard compliant and uses the same tree format as arch or debian 
(with the exceptions of /capture and /subspace)

# Are packages isolated?

-- No, and they will never be isolated , Heliade OS does not use weird path such as "/heliade/store"
...
-- Heliade OS supports multiple filesystem but bootstrapping the OS defaults to btrfs, without it,
the generation feature would not be usable.

# Will Heliade OS support systemd one day?
...
# Can you use Heliade OS imperatively?

-- Yes, however declaration is recommended to reproduce systems easily.

# Is Heliade OS beginner friendly?

-- No, Heliade OS is not a beginner friendly distro and needs to be manually installed and configured, it requires
